...
func TestFromDDl(t *testing.T) {
	...
	err = fromDDL("./user.sql", util.MustTempDir(), cfg, true, false, "tao")
	assert.Equal(t, errNotMatched, err)

	// case dir is not exists
	unknownDir := filepath.Join(util.MustTempDir(), "test", "user.sql")
	err = fromDDL(unknownDir, util.MustTempDir(), cfg, true, false, "tao")
	...
	// case empty src
	err = fromDDL("", util.MustTempDir(), cfg, true, false, "tao")
	if err != nil {
