	...
	if from.Kind() != reflect.Slice && fromType.Kind() == reflect.Map && toType.Kind() == reflect.Map {
		...
		for _, k := range from.MapKeys() {
			...
			if !set(toValue, from.MapIndex(k), opt.DeepCopy) {
				if err = deepCopy(toValue.Addr().Interface(), from.MapIndex(k).Interface(), opt); err != nil {
					return err
				}
			}
	...
	if from.Kind() == reflect.Slice && to.Kind() == reflect.Slice && fromType.ConvertibleTo(toType) {
		...
		for i := 0; i < from.Len(); i++ {
			...
			if !set(to.Index(i), from.Index(i), opt.DeepCopy) {
				...
				err = deepCopy(to.Index(i).Addr().Interface(), from.Index(i).Interface(), opt)
				if err != nil {
					continue
				}
			}
					...
					if toField.IsValid() {
						if toField.CanSet() {
							if !set(toField, fromField, opt.DeepCopy) {
								if err := deepCopy(toField.Addr().Interface(), fromField.Interface(), opt); err != nil {
									return err
								}
							}
							if fieldFlags != 0 {
			...
			if dest.Addr().Type().AssignableTo(to.Type().Elem()) {
				if to.Len() < i+1 {
					...
					if !set(to.Index(i), dest.Addr(), opt.DeepCopy) {
						// ignore error while copy slice element
						err = deepCopy(to.Index(i).Addr().Interface(), dest.Addr().Interface(), opt)
						if err != nil {
							continue
						}
					}
				...
				if to.Len() < i+1 {
					...
					if !set(to.Index(i), dest, opt.DeepCopy) {
						// ignore error while copy slice element
						err = deepCopy(to.Index(i).Addr().Interface(), dest.Interface(), opt)
						if err != nil {
							continue
						}
					}
